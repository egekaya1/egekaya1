...
|------|----------|--------|----------|
| 2025 | **Vigil** | Swift · SwiftUI · CoreML · PyTorch · C | Privacy-first system monitor that detects hardware anomalies (CPU hangs, memory leaks, thermal throttling) using on-device neural networks. Features a hybrid architecture: a low-level C collector for kernel metrics and a quantized Autoencoder running on the Apple Neural Engine for zero-overhead inference (<0.1% CPU). Includes a Python training pipeline for personalized baselining and a native SwiftUI dashboard for real-time visualization. Repository: https://github.com/egekaya1/Vigil |
